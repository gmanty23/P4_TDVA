...
with open(f'models/{modelo}/latent_spaces.npy', 'rb') as f:
    X = np.squeeze(np.load(f))
...
ax1.scatter(res[:, 0], res[:, 1], s=20) # c=y para colorear los puntos
...
if load_t_sne:
    ...
    res = tsne(X_scaled, T=3000, l=200, perp=40) # Me guardo res en el fichero t_sne.npy (para evitar recalcularlo)
    with open(f'models/{modelo}/t_sne.npy', 'wb') as f:
        np.save(f,res)

ax2.scatter(res[:, 0], res[:, 1], s=20) # c=y para colorear los puntos
