...
train_dataset_L, train_paths_L = load_spectrograms("inputs/spectrograms_"+route+"_L_"+case)
train_paths_L = [os.path.split(i)[-1].split('.pkl')[0] for i in train_paths_L]
train_dataset_R, train_paths_R = load_spectrograms("inputs/spectrograms_"+route+"_R_"+case)
...
graph = torch.load(f"models/{modelo}/model.pth",map_location=torch.device('cpu'))
...
with open(r"inputs/min_max_test/min_max_values.pkl", "rb") as input_file: # ¿Qué aspecto tienen las entradas de min_max_values.pkl?
...
with open(f'models/{modelo}/latent_test/latent_space_{route}_{case}.npy', 'wb') as f:
